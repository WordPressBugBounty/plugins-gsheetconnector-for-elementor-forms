...
class GSC_Elementor_Integration
{
    ...
    public function __construct()
    {
        ...
        add_action('wp_ajax_gs_elementor_reset_feed', array($this, 'gs_elementor_reset_feed'));
        ...
        // inner settings form submit entry going to the sheet
...
public function gscele_save_uninstall_settings()
{
    ...
    check_ajax_referer('gscele-elementor-setting-ajax-nonce', 'security');
    ...
    update_option('gscele_elementor_uninstall_settings_free', $setting);
 ...
 * @return void Sends JSON success or error response.
 */
function gs_elementor_reset_feed()
{
 ...
 *
 * It ensures that each feed name is unique per form and returns
...
public function save_gscelementor_feed()
{
    ...
    // Validate + unslash + sanitize
    $feedName = isset($_POST['feed_name'])
    ? sanitize_text_field(wp_unslash($_POST['feed_name']))
    : '';

    $elementorForms = isset($_POST['elementorForms'])
    ? intval(wp_unslash($_POST['elementorForms']))
    : 0;

    $message = '';

    if (!empty($feedName) && !empty($elementorForms)) {

        // ✅ use sanitized variables ONLY (no raw $_POST)
        $feed_check = get_post_meta($elementorForms, $feedName, true);

        if (empty($feed_check)) {
            update_post_meta($elementorForms, $feedName, 'gscele_form_feeds');
...
public function delete_feed()
{
    try {
        ...
        if ($feed_id) {
            ...
            if ($deleted1) {
             wp_send_json_success('Feed deleted successfully');
         } else {
            wp_send_json_error('Error deleting feed');
        }
    } else {
 ...
 * @param object $handler The Elementor handler object (not used directly but part of hook).
 *
 * @return void
 */
...
{
    try {
        if (isset($_POST['gsele-execute-edit-feed'])) {

            // ✅ Nonce check (unslash required)
            ...
            ? sanitize_text_field($elementor_gs['tab-id-custom'])
            : '';
...
private function maybe_migrate_gselef_feed_status() {
 ...
 $feedList = $wpdb->get_results(
    ...
    $wpdb->prepare("SELECT * FROM {$table} WHERE meta_value = %s", 'gscele_form_feeds')
);
...
if ( ! empty( $feedList ) ) {
    foreach ( $feedList as $value ) {
        ...
        // Update 'gselef_status' meta for each post
