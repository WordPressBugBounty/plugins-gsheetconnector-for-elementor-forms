...
/* --------------------------------------------------------------------------
 * Get all Elementor feeds
 *
 * Backward compatibility support:
 * OLD:
 * meta_value = 'gscele_form_feeds'
 *
 * NEW:
...
if ($show_setting == 1) {
    ...
    <div class="gscefp-feed-error-message" class="d-none"></div>
    <div class="gscefp-feed-success-message" class="d-none"></div>
    ...
    <div class="elementor-main">
                    ...
                    <div class="row">
                        ...
                        <div class="col-4 mt-25">
                            <div class="mr-10">
                                ...
                                <input type="button" name="execute-submit-feed-elementor" class="gselef-free-feed-sub-btn btn btn-primary" value="<?php echo esc_attr__('Submit', 'gsheetconnector-for-elementor-forms'); ?>">
        ...
        <div class="elementor-feeds-list form-feed-table-common mt-30">
            <table id="elementorformtable" class="full-table">
                ...
                if (!empty($current_feeds)) {
                    ...
                    <th class="pt-15 pb-15 pl-20 pr-20 text-left bg-white text-dark fw-600">' . esc_html__('Form ID', 'gsheetconnector-for-elementor-forms') . '</th>
        ...
        if (
            is_array($meta_value) &&
            isset($meta_value['element_id'])
        ) {

            $feed_element_id = sanitize_text_field(
                $meta_value['element_id']
            );
        }

        /* ------------------------------------------
         * NEW feeds -> element_id based
         * ------------------------------------------ */
        if (!empty($feed_element_id)) {

            $form_result = get_form_by_element_id(
                $data,
                $feed_element_id
            );
...
<td class="pt-15 pb-15 pl-20 pr-20 text-left gselef-free_elementorform_status">
<div class="custom-check">

<input type="checkbox"
class="check-toggle gselef-status-toggle"
id="gselef-free_elementorform_status_' . esc_attr($value->meta_id) . '"
data-feed-id="' . esc_attr($value->meta_id) . '"
value="1"
' . checked($status, 1, false) . '>

<label for="gselef-free_elementorform_status_' . esc_attr($value->meta_id) . '" class="button-toggle"></label>
...
<!--  Delete feed popup start -->
<div id="gselef-free-delete-feed-confirm-popup" class="gselef-free-delete-feed-popup-overlay d-none">
    <div class="gselef-free-delete-feed-popup text-center">
    ...
    <div class="gsc-modal-title">
        <?php echo esc_html__('Delete Feed', 'gsheetconnector-for-elementor-forms'); ?>
    </div>

    <p class="gsc-modal-text">
        <?php echo esc_html__('Are you sure you want to Delete Feed.', 'gsheetconnector-for-elementor-forms'); ?>
    </p>

    <div class="popup-actions d-flex justify-center gap-10">

        <button type="button" class="btn deactivate-btn" id="gselef-free-delete-feed-popup-cancel">
            <?php echo esc_html__('Cancel', 'gsheetconnector-for-elementor-forms'); ?>
        </button>

        <button type="button" class="btn btn-primary" id="gselef-free-delete-feed-popup-confirm">
            <?php echo esc_html__('Delete', 'gsheetconnector-for-elementor-forms'); ?>
        </button>
