jQuery(window).on("load", function () {
  elementor.channels.editor.on("namespace:editor:gsceviewsheet", function () {
    ...
    let tabId = jQuery(
      '[data-setting="gs_spreadsheet_tab_name"] option:selected',
      ).val();
  ...
  elementor.channels.editor.on(
    "namespace:editor:gsceviewsheetmanual",
    function () {
      let msheetId = jQuery('[data-setting="manual_sheet_id"]').val();

      let mtabId = jQuery('[data-setting="manual_tab_id"]').val();
      ...
      const url_m = `https://docs.google.com/spreadsheets/d/${msheetId}/edit#gid=${mtabId}`;
...
//     if (
//       changed.gs_spreadsheet_id != "" &&
//       changed.gs_spreadsheet_id != undefined
//       ) {
//       var sheetnameId = jQuery("[data-setting = gs_spreadsheet_id]").val();
//     var tabs_arr_val = jQuery(
//       "[data-setting = gs_elmentor_all_sheet_data]",
//       ).val();
...
// jQuery(document).on(
//   "change",
//   "[data-setting='gs_spreadsheet_id']",
//   function () {

//     const sheetnameId = jQuery(this).val();

//     if (!sheetnameId || sheetnameId === currentSheetId) {
//       return;
//     }

//     currentSheetId = sheetnameId;
...
//         );
//       options_selected_sheet(sheetnameId);
//       jQuery("[data-setting =  gs_spreadsheet_tab_name]").attr(
//         "disabled",
//         false,
//         );
...
jQuery(document).on(
  ...
  function () {
    ...
    // Prevent duplicate requests for same sheet
    if (sheetnameId === currentSheetId) {
      return;
    }

    currentSheetId = sheetnameId;

    // Check if tab data already exists
    let tabsData = jQuery(
      "[data-setting='gs_elmentor_all_sheet_data']"
    ).val();
    ...
    if (tabsData) {
      ...
      try {
        ...
        if (
          tabsData[sheetnameId] &&
          Object.keys(tabsData[sheetnameId]).length > 0
        ) {
        ...
        console.warn("Invalid cached tab data");
      }
    }

    fetch_tabs_api(sheetnameId);
  }
);

function fetch_tabs_api(sheetnameId) {

  jQuery(".loading-gs-fetch-tabs").addClass("loading");

  jQuery("[data-setting='gs_spreadsheet_tab_name']")
    .prop("disabled", true);

  jQuery(".tabselectionloading")
  ...
  const data = {
    ...
    refresh: 0,
    security: jQuery(
      "[data-setting='gs-ajax-nonce-ele']"
    ).val(),
  };
...
function options_selected_sheet(sheetnameId) {
  ...
  if (
    typeof all_tab_array === "string" &&
    all_tab_array.trim().charAt(0) === "{"
    ) {
    try {
      all_tab_array = JSON.parse(all_tab_array);
    } catch (e) {
      console.error(
        "Invalid JSON in gs_elmentor_all_sheet_data:",
        all_tab_array,
        );
      return;
    }
  ...
  if (typeof all_tab_array !== "object") {
    console.error(
      "gs_elmentor_all_sheet_data is not a valid object:",
      all_tab_array,
      );
    return;
  }
